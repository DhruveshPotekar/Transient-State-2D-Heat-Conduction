function [ T ] = transient_implicit_gs(nt,error,tol,nx,ny,T,Told,T_previous_nt,term1,term2,term3)
...
%time loop
  ...
  for k = 1:nt
   ...
   figure(1);
   semilogy(k,error1,'marker','*','color','g');
   hold on;
   ...
   pause(0.001)
    ...
    title_text = sprintf('Transient heat conduction implicit equation using gauss seidel method');
    title(title_text)
    xlabel('x');
    ylabel('y');
